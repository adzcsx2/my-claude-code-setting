...
function redactSecrets(value) {
  return String(value || '')
    ...
    .replace(/(^|\s)(--user)(?:=|\s+)[^:\s]+:[^\s]+/gi, '$1$2 <REDACTED>')
    .replace(/(^|\s)(-u)(?:\s+)?[^:\s]+:[^\s]+/gi, '$1$2 <REDACTED>')
    .replace(/\b(https?:\/\/)[^/\s:@]+:[^/\s@]+@/gi, '$1<REDACTED>@')
    .replace(/\b([A-Z0-9_]*(?:API_KEY|AUTH_TOKEN|ACCESS_TOKEN|SECRET_KEY|PRIVATE_KEY))=(\"[^\"]*\"|'[^']*')/gi, '$1=<REDACTED>')
    .replace(/\b([A-Z0-9_]*(?:API_KEY|AUTH_TOKEN|ACCESS_TOKEN|SECRET_KEY|PRIVATE_KEY))=(['"]?)\S+/gi, '$1=<REDACTED>')
    .replace(/\bAKIA[A-Z0-9]{16}\b/g, '<REDACTED>')
    .replace(/\bASIA[A-Z0-9]{16}\b/g, '<REDACTED>')
    .replace(/\bsk-[A-Za-z0-9_-]{10,}\b/g, '<REDACTED>')
    .replace(/\bxox[beaprs]-[A-Za-z0-9-]+\b/g, '<REDACTED>')
...
function patchPreviewFromGitDiff(repoRoot, pathCandidates) {
  for (const candidate of pathCandidates) {
    ...
    if (relevant.length > 0) {
      return sanitizePatchLines(relevant.join('\n'), MAX_RELEVANT_PATCH_LINES).join('\n');
    }
  }
